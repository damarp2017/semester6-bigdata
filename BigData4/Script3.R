...
sample_pie = penjualan_provinsi %>%
  select(Tanggal, Province, Total_Penjualan) %>%
  group_by(Tanggal, Province) %>%
  summarise(Total_Penjualan = sum(Total_Penjualan))

vis = ggplot(sample_pie, aes(x = "", y=Total_Penjualan, fill=Province))+geom_bar(stat = "identity")

vis
vis + coord_polar("y", start = 4)
